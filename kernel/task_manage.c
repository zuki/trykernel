...
TCB	tcb_tbl[CNF_MAX_TSKID];         /* タスク管理ブロック (TCB)の配列 */
...
ID tk_cre_tsk( const T_CTSK *pk_ctsk )
{
    ...
    /* TCBの初期化 */
    if(i < CNF_MAX_TSKID) {
        tcb_tbl[i].state    = TS_DORMANT;       // 休止状態で作成
        tcb_tbl[i].pre      = NULL;             // キューには入っていない
        tcb_tbl[i].next     = NULL;             // キューには入っていない
...
void tk_ext_tsk( void )
{
    ...
    tqueue_remove_top(&ready_queue[cur_task->itskpri]); // カレントタスクを待ち行列から削除
